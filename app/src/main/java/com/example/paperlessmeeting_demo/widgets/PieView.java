...
public class PieView extends View {
    ...
    //延长点的大小
    private float smallCircleRadius = 6F;
    //延长点上的同心圆环的大小
    private float bigCircleRadius = 10F;
    //延长线转折点的横向偏移
    private float xOffset = 30F;
    //延长线转折点的纵向偏移
    private float yOffset = 50F;
    //延长线最长段部分的长度
    private float extend = 250F;
    ...
    private boolean isAnonymity = true;  //  是否匿名
    private ArrayList<Region> mRegionLists = new ArrayList<>();
    private Toast toast;
    ...
    private void initRectF() {
        ...
        mCenterCircleX = mTotalWidth / 2;
        mCenterCircleY = mTotalHeight / 2;
    }
...
//            drawHole(canvas);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()){
            case MotionEvent.ACTION_DOWN:
                if (!isAnonymity){
                    for (Region region : mRegionLists) {
                        if(region.contains(Math.round(x-mCenterCircleX),Math.round(y-mCenterCircleY))) {
                            List list = mPieLists.get(mRegionLists.indexOf(region)).getChooseUserList();
                            String text =list.toString().replace("[","").replace("]","");
                            showMyPieTost(text,x,y);
                            break;
                        }
                    }
                }
    ...
    private void showMyPieTost(String text,float x,float y) {
        ...
        if (topActivity != null) {
            ...
            textView.setText("投票人员:\n"+text);

//            Log.d("x-mCenterCircleX","x-mCenterCircleX==="+ (x-mCenterCircleX));
            if(toast == null){
                toast = new Toast(MeetingAPP.getContext());
            }
            toast.setDuration(Toast.LENGTH_LONG);
            toast.setGravity(Gravity.CENTER, Math.round(x-mCenterCircleX+30), Math.round(y-mCenterCircleY+30));
    ...
    private void drawPie(Canvas canvas) {
        ...
        for (PieEntry pie : mPieLists) {
            ...
            // 以下方法就可以画出来了，但是为了判断点击事件，采用path绘制
//            canvas.drawArc(mRectF,pie.getCurrentStartAngle(),pie.getSweepAngle(),true, mPaint);

            Path ovalPath = new Path();
            Region mRegion = new Region();
            ovalPath.moveTo(mCenterCircleX, mCenterCircleY);
            ovalPath.lineTo( mCenterCircleX+mRadius*(float)Math.cos(pie.getCurrentStartAngle()), mCenterCircleY+(float)(mRadius*Math.sin(pie.getCurrentStartAngle())));
            ovalPath.addArc(mRectF, pie.getCurrentStartAngle(), pie.getSweepAngle());
            ovalPath.close();
            RectF r = new RectF();
            ovalPath.computeBounds(r, true);
            mRegion.setPath(ovalPath, new Region((int)r.left, (int) r.top, (int) r.right,(int)r.bottom));
            mRegionLists.add(mRegion);
