...
public class HttpDownloader {
    ...
    public int downloadFiles(String urlStr, String path, String fileName) {
        try {
            ...
            if (DownloadFileUtils.isFileExist(fileName, path)) return 1;//判断文件是否存在
            else {
                InputStream inputStream = getInputStreamFromUrl(urlStr);
                File resultFile = DownloadFileUtils.write2SDFromInput(fileName, path, inputStream);
