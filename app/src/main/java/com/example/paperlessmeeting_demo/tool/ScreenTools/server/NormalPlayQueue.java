...
public class NormalPlayQueue {
    private ArrayBlockingQueue<DecodeFrame> mPlayQueue;
    ...
    public NormalPlayQueue() {
        ...
        mScanThread.start();
        mPlayQueue = new ArrayBlockingQueue<DecodeFrame>(NORMAL_FRAME_BUFFER_SIZE, true);
    }


    public DecodeFrame takeByte() {
        try {
            DecodeFrame frame = mPlayQueue.take();
            showLog("playqueue count = " + mPlayQueue.size());
            if (frame.getType() == DecodeFrame.KEY_FRAME) mKeyFrameCount.getAndDecrement();
    ...
    public void putByte(DecodeFrame frame) {
        if (frame.getType() == DecodeFrame.KEY_FRAME) mKeyFrameCount.getAndIncrement();
        abandonData();
        try {
    ...
    private void abandonData() {
        if (mTotalFrameCount.get() >= (mFullQueueCount / 3)) {
            ...
            boolean start = false;
            for (DecodeFrame frame : mPlayQueue) {
                if (!start) showLog("丢掉了下一个KEY_FRAME前的所有INTER_FRAME..");
                if (frame.getType() == DecodeFrame.NORMAL_FRAME) {
                    start = true;
                }
                if (start) {
                    if (frame.getType() == DecodeFrame.NORMAL_FRAME) {
                        ...
                        pFrameDelete = true;
                    } else if (frame.getType() == DecodeFrame.KEY_FRAME) {
            ...
            if (!pFrameDelete) {
                for (DecodeFrame frame : mPlayQueue) {
                    if (frame.getType() == DecodeFrame.KEY_FRAME) {
                        mPlayQueue.remove(frame);
                        Log.d(TAG, "丢掉了一个关键帧..");
                        mTotalFrameCount.getAndDecrement();
                        mGiveUpFrameCount.getAndIncrement();
            ...
            if (!pFrameDelete && !kFrameDelete) {
                for (DecodeFrame frame : mPlayQueue) {
                    if (frame.getType() == DecodeFrame.AUDIO_FRAME) {
                        mPlayQueue.remove(frame);
                        mTotalFrameCount.getAndDecrement();
                        mGiveUpFrameCount.getAndIncrement();
                        break;
