 ...
 */
/*
 *文件分享和推送旧版本功能，去除MD5验证文件是否存在，和WuHuActivity2  PagerAdapter2配合使用
 */
public class WuHuFragment2 extends BaseFragment implements MediaReceiver.sendfilePthInterface, WuHuFileListAdapter.upLoadFileInterface, WuHuFileListAdapter.PlayerClickInterface, WuHuFileListAdapter.ShareFileInterface, WuHuFileListAdapter.PushFileInterface, View.OnClickListener {
    Unbinder unbinder;
 ...
 /*   @BindView(R.id.tv_test)
    ...
    private boolean FileIsShare = false;//文件有没有点过分享
    private boolean FileIsPush = false;//推送按钮有没点过
   ...
   /*     //第一个fragment只显示会议目录
        ...
        listView_catalog.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Log.d("wvdfsdfgsdg44444", "CHANGE_CATALOG_BROADCAST");
...
/*    //获取网络下载到的文件
    ...
    private void UiaSsign(List<WuHuEditBean.EditListBean> editListBeanList) {
        ...
        if (attend2 != null) {
            ...
            // Log.d("fdafafaff=fragment2222", "议题 " + textNub +"    "+editListBeanList.size()+ "     对应的列席单位：" + editListBeanList.get(Integer.valueOf(textNub)).getParticipantUnits());
        }
            ...
            if (shareResponNum == 1) {
                Thread sendThread = new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Log.d("vvcvsvsfgsf-ip大小：", stringListIp.size() + "  ");
                        //根据Ip将文件发送到不同的设备
                        for (int i = 0; i < stringListIp.size(); i++) {
                            Log.d("vvcvsvsfgsf-每一个ip：", stringListIp.get(i) + "    大小：" + stringListIp.size());
...
//63202ec788ccdf4b7e2b8f7f
    }

    public static WuHuFragment2 newInstance(String text) {
        WuHuFragment2 wuHuFragment = new WuHuFragment2();
        Bundle bundle = new Bundle();
