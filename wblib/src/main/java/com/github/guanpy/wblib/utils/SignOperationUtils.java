...
public class SignOperationUtils {
    /**
     * 操作类型-画笔
     */
     ...
     * 单例
     */
    private static SignOperationUtils mOperationStack;
    /**
     * 当前是否禁止白板操作
     */
    public boolean DISABLE = true;

//    public boolean MyDISABLE = true;
    /**
     * 当前所在白板位置
     */
     ...
     * 白板操作集的集合,用以存储不同白板
     */
    private List<WhiteBoardPoints> mWhiteBoardPointList = new ArrayList<>(3);
    /**
     * 单个白板操作
     */
    private DrawPoint msingleWhiteBoardPoint;
    /**
     * 单例
     */
    public static SignOperationUtils getInstance() {
        if (mOperationStack == null) {
            mOperationStack = new SignOperationUtils();
        }
    ...
    public void init() {
        ...
        SignOperationUtils.getInstance().initDrawPointList();
    }
     ...
     * 返回指定白板
     */
    public List<WhiteBoardPoints> getmWhiteBoardPointList() {
        return mWhiteBoardPointList;
    }

    // 替换指定位置的数据集
    public void setmWhiteBoardPointList(WhiteBoardPoints mWhiteBoardPoint,int index) {
        this.mWhiteBoardPointList.set(index,mWhiteBoardPoint);
    }

    public void setmWhiteBoardPoints(WhiteBoardPoints mWhiteBoardPoints) {
        this.mWhiteBoardPoints = mWhiteBoardPoints;
    }

    /**
     * 返回指定白板的单个操作
     */
    public DrawPoint getMsingleWhiteBoardPoint() {
        return msingleWhiteBoardPoint;
    }
    /**
     * 设置指定白板的单个操作
     */
    public void setMsingleWhiteBoardPoint(DrawPoint msingleWhiteBoardPoint) {
        this.msingleWhiteBoardPoint = msingleWhiteBoardPoint;
    }
    ...
    public void initDrawPointList() {
        ...
        for(int i= 0;i<3;i++){
            WhiteBoardPoints WhiteBoardPoints = new WhiteBoardPoints();
            WhiteBoardPoints.setId(i+"");
            WhiteBoardPoints.setWhiteBoardPoints(new ArrayList<WhiteBoardPoint>());
            this.mWhiteBoardPointList.add(i,WhiteBoardPoints);
        }
    }
    ...
    public List<DrawPoint> getSavePoints() {
        return SignOperationUtils.getInstance().getDrawPointList(SignOperationUtils.getInstance().mCurrentIndex).getSavePoints();
    }
    ...
    public List<DrawPoint> getDeletePoints() {
        return SignOperationUtils.getInstance().getDrawPointList(SignOperationUtils.getInstance().mCurrentIndex).getDeletePoints();
    }
